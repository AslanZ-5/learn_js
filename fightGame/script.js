...
class Player {
  ...
  strike (player, enemy, attackDmg) {
    ...
    let dm = Math.ceil(Math.random() * attackDmg)

    // Subtract the enemy health with the damageAmount
    enemy.health -= dm
    updateGame(p1,p2,game.isOver)
    console.log('p',player.health,'e', enemy.health)
    return `${player.name} attacks ${enemy.name} for ${dm}`
...
let player1 = new Player('asad',100,10)
let player2 = new Player('mus1ab',100,10)
console.log(player1)
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key == 'a' && game.isOver == false && p1.health > 0){
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key == 'l' && game.isOver == false && p2.health > 0){
